...
const GOVDE = /metin kismi|metin kisimlari|ana metin|tüm metin|tez metni|gövde metni|gövde|düz metin|normal metin/gu;
...
*/
const GOVDE_DISI = /başlik|dipnot|kapak|özet|abstract|içindekiler|tablo|çizelge|şekil|resim|grafik|alinti|kaynak|sayfa numara|jüri|beyan|teşekkür|simge|kisaltma|ek(?:ler)?\b/gu;

/** Deseni metnin tamamında arar; son eşleşmenin konumu, yoksa -1. */
function sonKonum(metin: string, desen: RegExp): number {
...
export function govdeOlcusu(
  ...
  while ((eslesme = arayici.exec(compact)) !== null) {
    ...
    if (sonKonum(onceki, GOVDE_DISI) > sonKonum(onceki, GOVDE)) continue;
